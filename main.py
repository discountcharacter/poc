...
def main():
    ...
    if st.button("Calculate Value", type="primary", use_container_width=True):
        ...
        # Load proxy codes and display car info
        try:
            proxy_df = pd.read_csv('data/car_model_proxy_codes.csv')
            car_match = proxy_df[(proxy_df['make'].str.lower() == make.lower()) & 
                                  (proxy_df['model'].str.lower().str.contains(model.lower()))]
            
            if not car_match.empty:
                proxy_code = car_match.iloc[0]['proxy_code']
                
                # Display car info banner
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                           padding: 2rem; border-radius: 15px; margin-bottom: 2rem;
                           color: white; text-align: center;">
                    <h2 style="margin: 0; font-size: 2.5rem; font-weight: 800;">
                        {year} {make.title()} {model.title()}
                    </h2>
                    <div style="font-size: 4rem; font-weight: 900; margin: 1rem 0;
                               font-family: monospace; letter-spacing: 5px;">
                        {proxy_code}
                    </div>
                    <p style="margin: 0; font-size: 1.2rem; opacity: 0.9;">Proxy Code</p>
                </div>
                """, unsafe_allow_html=True)
        except Exception as e:
            print(f"Proxy code lookup error: {e}")
...
def run_valuation(make, model, year, variant, km, condition, owners, fuel, location, remarks, gemini_key, search_key, cx):
    ...
    with st.spinner("Orchestrating Intelligent Valuation..."):
        ...
        # 5. Oracle - DISABLED (no longer uses external API)
        ...
        oracle_price = None
        oracle_debug = {"status": "disabled", "message": "Oracle engine disabled - no longer requires external API"}
